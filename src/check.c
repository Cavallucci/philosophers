...
/*   Updated: 2022/01/26 18:06:19 by lcavallu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_data(t_data *d, char **argv)
{
	...
	d->forks = (pthread_mutex_t *)ft_calloc(sizeof(pthread_mutex_t),
			d->nb_philo);
	if (!d->forks)
		return (ERROR);
...
int	init_mutex(t_data *d)
{
	...
	i = 0;
	if (pthread_mutex_init(&d->mutex_die, NULL) != 0
		|| pthread_mutex_init(&d->mutex_print, NULL) != 0
		|| pthread_mutex_init(&d->mutex_max_eat, NULL) != 0)
	{
		ft_free(d, d->philo);
		return (ERROR);
	}
